...
#[tokio::test]
async fn get_all_groups_returns_200_and_all_groups_when_user_is_admin(
    app: &TestApp,
) -> anyhow::Result<()> {
    // Arrange
    let admin = app
        ...
        .await?;
    let group_1 = app
        .create_user_and_group("rbiland1", "r1@r.com", "201", "my group")
        .await?;
    let group_2 = app
        .create_user_and_group("rbiland1", "r2@r.com", "201", "my group 2")
        .await?;
    let group_3 = app
    ...
    let response = app
        .client
        .get(&format!("{}/admin/groups", &app.address))
    ...
    assert_eq!(body.data.groups.len(), 3);
    let grp_1 = body.data.groups.iter().find(|grp| grp.id == group_1.id);
    assert_some!(grp_1);
    let grp_2 = body.data.groups.iter().find(|grp| grp.id == group_2.id);
    assert_some!(grp_2);
    ...
    assert_eq!(admin_1.name, "rbiland1");
...
#[test_context(TestApp)]
#[tokio::test]
async fn get_groups_returns_200_and_an_empty_list_if_there_are_no_groups(
    app: &TestApp,
) -> anyhow::Result<()> {
    // Arrange
    let user = app
...
#[test_context(TestApp)]
#[tokio::test]
async fn get_groups_returns_403_if_user_is_not_admin(app: &TestApp) -> anyhow::Result<()> {
    // Arrange
    let user = app
        .create_user_and_login_and_device("rbiland", "r@r.com", "201")
        .await?;
